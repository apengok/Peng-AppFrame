...
class FieldConverter(object):
    """
        Helper class that coverts model fields into WTForm fields

        it has a conversion table with type method checks from model interfaces,
        there methods are invoked with a column name.
    """
...
class GeneralModelConverter(object):
    """
        Returns a form from a model only one public exposed method 'create-form'
    """
    ...
    def _convert_many_to_one(self, col_name, label, description,
        ...
        """
            ...
            will only work with SQLAlchemy interface.
        """
    ...
    def _convert_col(self,col_name,label,description,lst_validators,filter_rel_fields,form_props):
    ...
    def create_form(self,label_columns=None,inc_columns=None,description_columns=None,
            validators_columns=None,extra_fields=None,filter_rel_fields=None):
        """
            Converts a model to a form given

            :param label_columns:A dictionary with the column's label
            :param inc_columns: A list with the columns to include
            :param description_columns:A dictionary with a description for cols
            :param validators_columns:
                A dictionary with WTForms validators ex::

                    validators={'personal_email':EmailValidator}
            :param extra_fields:
                A dictionary containing column names and a WTForm Form fields to be added to the form,
                these fields do not exist on teh model itself ex::
                    extra_fields={'some_col':BooleanField('Some Col',default=False)}
            :param filter_rel_fields:A filter to be applied on relationships
        """
