...
class AuditMixin(object):
    """
        AuditMixin
        Mixin for models,adds 4 columns to stamp,time and user on createion and modification
    ...
    changed_on = Column(DateTime,default=datetime.datetime.now,onupdate=datetime.datetime.now,nullable=False)
    ...
    @declared_attr
    def changed_by(cls):
        return relationship("User",primaryjoin='%s.changed_by_fk == User.id' % cls.__name__,enalble_typechecks=False)
