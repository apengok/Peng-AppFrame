...
class SQLA(SQLAlchemy):
    """
        ...
        it's purpose is to override the declarative base of the original package.
        so that it is bound to F.A.B Model class allowing the dev to be in the same
        namespace of the ecurity tables(and others) and can use AuditMixin class alike.

        Use it and configure it just ike flask_SQLAlchemy
    """

    def make_declarative_base(self,metadata=None):
        """Creates the declarative base."""
        base = declarative_base(cls=Model,name='Model',metadata=metadata,metaclass=ModelDeclarativeMeta)
