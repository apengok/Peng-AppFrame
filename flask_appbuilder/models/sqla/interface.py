...
from ..const import LOGMSG_ERR_DBI_ADD_GENERIC,LOGMSG_ERR_EBI_EDIT_GENERIC,LOGMSG_ERR_DBI_DEL_GENERIC,\
        LOGMSG_WAR_DBI_ADD_INTEGRITY,LOGMSG_ERR_DBI_EDIT_INTEGRITY,LOGMSG_WAR_DBI_DEL_INTEGRITY
...
class SQLAInterface(BaseInterface):
    ...
    def __init__(self,obj,session=None):
        ...
        #collect all AQLA columns and properties
    ...
    def query(self,filters=None,order_column='',order_direction='',page=None,page=None,page_size=None):
        ...
        if len(order_column.split('.')) >= 2:
            ...
            for join_relation in order_column.split('.')[:-1]:
                ...
                # redefine order column name,because relationship can have a diffrent name
    ...
    def query_year_group(self,group_by='',filters=None):
        ...
        query = self._get_base_query(query=query,filters=filters)
        query_result = query.all()
        goup_year = GroupByDateYear(group_by,'Group by Year')
    ...
    def is_relation_one_to_many(self,col_name):
        try:
            if self.is_relation(col_name):
                return self.list_columns[col_name].direction.name == 'ONETOMANY'
            except:
                return False
    ...
    def add(self,item):
        try:
            ...
            log.warning(LOGMSG_WAR_DIB_ADD_INTEGRITY.format(str(e)))
    ...
    def edit(self,item):
        try:
            ...
            log.warning(LOGMSG_WAR_DBI_INTEGRITY.format(str(e)))
            self.session.rollback()
            return False
        except Exception as e:
            self.message = (as_unicode(self.general_error_message + ' ' + str(sys.exc_info()[0])),'danger')
    ...
    def get_search_columns_list(self):
        ...
        for col_name in self.get_columns_list():
            if not self.is_relation(col_name):
                tmp_prop = self.get_property_list_col(col_name).name
    ...
    def get_image_column_list9self):
    ...
    def get_property_first_col(self,col_name):
        #supprt for only one col for pk and fk
