...
class BaseProcessData(object):
    """
        ...
        The aggregation is made by an already defined funcion,or by a custom funcion
    ...
    def resolve_attr(self,obj,attr):
        if not hasattr(obj,attr):
            return reduce(getattr,attr.split('.'),obj) #its an inner obj attr
        if hasattr(getattr(obj,attr),'__call__'):
            return getattr(obj,attr)() #its a function
        else:
            return getattr(obj,attr)  #its an attribute
...
class DirectProcessData(BaseProcessData):
    ...
    def apply(self,data,sort=True):
        ...
        if sort:
            data = sorted(data,key=attrgetter(group_by))
        result = []
        for item in data:
