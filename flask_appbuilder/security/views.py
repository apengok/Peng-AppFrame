...
class UserModelView(ModelView):
    ...
    description_columns = {'first_name':lazy_gettext('Write the user first name or names'),
            ...
            'username':lazy_gettext('Username valid for authentic on DB or LDAP,unused for OID auth'),
    ...
    show_fieldsets = [
            (lazy_gettext('User info'),{'fields':['username','active','roles','login_count']}),
            (lazy_gettext('Personal info'),{'fields':['first_name','last_name','email'],'expanded':True}),
            (lazy_gettext('Audit Info'),{'fields':['last_login','fail_login_count','created_on','created_by','changed_on','changed_by'],'expanded':False}),
            ]

    user_show_fieldsets = [
            (lazy_gettext('User omfo'),{'fields':['username','active','roles','login_count']}),
            (lazy_gettext('personal info'),{'fields':['first_name','last_name','email'],'expanded':True}),
            ]
...
class UserDBModelView(UserModelView):
    ...
    add_form_extra_fields = {'password':PasswordField(lazy_gettext('Password'),
        description=lazy_gettext('Please use a good password policy,this application does not check this for you'),validators=[validators.DataRequired()],widget=BS3PasswordFieldWidget()),
        'conf_password':PasswordField(lazy_gettext('Confirm Password'),
            description=lazy_gettext('Please rewrite the user\'s password to confirm'),validators=[EqualTo('password',message=lazy_gettext('Passwords must match'))],widget=BS3PasswordFieldWidget())}
    ...
    def show(self,pk):
        ...
        self.update_redirect()
        return self.render_template(self.show_template,pk=pk,
