...
class FileManager(object):
    def __init__(self,base_path=None,
        ...
        if not allowed_extensions and 'FILE_ALLOWED_EXTENSIONS' in ctx.app.config:
            self.allowed_extensions = ctx.app.config['FILE_ALLOWED_EXTENSTIONS']
        else:
...
class ImageManager(FileManager):
    ...
    def get_url_thumbnail(self,filename):
        if isinstance(filename,FileStorage):
            return filename.filename
        return self.relative_path + thumbge_filename(filename)
...
def get_file_original_name(name):
    """
        ...
        :param name: The file name from model
        :return: Returns the user's original filename removes <UUID>_sep_
    """
